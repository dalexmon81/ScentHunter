...
# ============================================================
# This route is intentionally defined in main.py itself.
# It does not import diagnostic_pipeline_endpoint.py.
# It reports where candidates disappear in the real pipeline.
...
def diagnostic_search_candidates(
    ...
    try:
        for future in as_completed(futures, timeout=max(1.0, float(timeout))):
            ...
            try:
                ...
                candidates = [
                    dict(x) for x in (result if isinstance(result, list) else [])
    ...
    for candidate in global_candidates:
        ...
        if store in by_store:
            by_store[store].append(candidate)
    ...
    for store in STORES:
        ...
        for candidate in by_store.get(store, []):
            row = {
                "name": product_field(
                    candidate, "name", "title", "product_name"
                ),
                "url": candidate.get("url", ""),
                "price": candidate.get("price", ""),
                "store": store,
                "store_product_id": identity_value(
                    candidate, "store_product_id", "product_id", "id"
                ),
                "matches": False,
                "catalog_match": False,
                "final": False,
            ...
            try:
                ...
                row["reason"] = f"matches_error: {type(exc).__name__}: {exc}"
                rows.append(row)
                continue
            ...
            try:
                catalog_product = _catalog_match(candidate, query)
            except Exception as exc:
                row["reason"] = (
            ...
            if catalog_product is not None:
                ...
                row["family_id"] = catalog_product.get("family_id", "")
                ...
                accepted.append(catalog_product)
                rows.append(row)
                continue

            family = _catalog_family_for_query(query)
            if family is not None:
                row["reason"] = "rejected_no_authoritative_variant_match"
                rows.append(row)
                continue

            try:
                generic = _apply_generic_display_name(candidate, query)
            except Exception as exc:
                row["reason"] = (
        ...
        stages[store] = {
            ...
            "global_dedup_count": len(by_store.get(store, [])),
            "matched_count": sum(1 for r in rows if r["matches"]),
            "catalog_matched_count": sum(
                1 for r in rows if r["catalog_match"]
            ),
            "final_count": sum(1 for r in rows if r["final"]),
            "rejected_count": sum(1 for r in rows if not r["final"]),
            "rows": rows,
        }

    propagated = _propagate_catalog_identity(accepted)
