...
# ============================================================
# SEARCH DIAGNOSTIC ENDPOINT
# ============================================================
#
# Read-only diagnostic. It does NOT alter normal search behaviour.
# It executes the real store discovery functions and then reports,
# per store, where candidates disappear:
# discovery -> local dedup -> matches() -> catalog_match -> final.
#

@app.get("/diagnostic-search-candidates")
def diagnostic_search_candidates(
    q: str = "Hawas",
    timeout: float = 120.0,
):
    ...
    try:
        ...
        for future in completed:
            ...
            try:
                ...
                # run_store() already performs the scraper-attempt local
                # deduplication, so this is the actual candidate pool
                # entering central validation.
                store_candidates = [
                    dict(item)
    ...
    # Global deduplication, exactly as the normal orchestrator starts.
    global_candidates = unique_results(all_candidates)

    by_store_global: Dict[str, List[Dict[str, Any]]] = {
        store: []
        for store in STORES
    }

    for candidate in global_candidates:
        store = str(candidate.get("store") or "").strip().lower()
        if store in by_store_global:
            by_store_global[store].append(candidate)

    stages: Dict[str, Any] = {}

    # Reproduce central validation one candidate at a time so we can report
    # the exact reason instead of silently losing rejected candidates.
    validated: List[Dict[str, Any]] = []

    for store in STORES:
        raw = discovery.get(store, {}).get("candidates", [])
        global_store = by_store_global.get(store, [])

        rows: List[Dict[str, Any]] = []

        for candidate in global_store:
            row = {
            ...
            try:
                row["matches"] = bool(matches(candidate, query))
            except Exception as exc:
                row["reason"] = (
                    f"matches_error: {type(exc).__name__}: {exc}"
                )
                rows.append(row)
                continue

            if not row["matches"]:
                row["reason"] = "rejected_by_matches"
                rows.append(row)
                continue
            ...
            try:
                ...
                row["reason"] = (
                    f"catalog_match_error: {type(exc).__name__}: {exc}"
                )
                rows.append(row)
                continue

            if catalog_product is not None:
                row["catalog_match"] = True
                row["family_id"] = catalog_product.get(
                    "family_id", ""
                )
                row["catalog_variant"] = catalog_product.get(
                    "catalog_variant",
                    catalog_product.get("canonical_name", ""),
                )
                row["canonical_name"] = catalog_product.get(
                    "canonical_name", ""
                )
                validated.append(catalog_product)
                row["final"] = True
                row["reason"] = "accepted_catalog_match"
                rows.append(row)
                continue
            ...
            if family is not None:
                row["reason"] = (
                    "rejected_no_authoritative_variant_match"
                )
                rows.append(row)
                continue

            try:
                generic = _apply_generic_display_name(
                    candidate,
                    query,
                )
            except Exception as exc:
                row["reason"] = (
                    f"generic_match_error: {type(exc).__name__}: {exc}"
                )
                rows.append(row)
                continue
            ...
            if isinstance(generic, dict):
                ...
                row["final"] = True
                row["reason"] = "accepted_generic_match"
            else:
                row["reason"] = "rejected_generic_match"

            rows.append(row)

        stages[store] = {
            "discovery_count": len(raw),
            "global_dedup_count": len(global_store),
            "matched_count": sum(
                1 for row in rows if row["matches"]
            ),
            "catalog_matched_count": sum(
                1 for row in rows if row["catalog_match"]
            ),
            "final_count": sum(
                1 for row in rows if row["final"]
            ),
            "rejected_count": sum(
                1 for row in rows if not row["final"]
            ),
            "rows": rows,
        }

    # Apply the same identity propagation + grouping used by normal search.
    propagated = _propagate_catalog_identity(validated)
    final_groups = _group_catalog_results(propagated)
    ...
    return {
        ...
        "summary": {
            ...
            "global_unique_candidates": len(global_candidates),
            "validated_candidates": len(validated),
            "propagated_candidates": len(propagated),
            "final_product_cards": len(final_groups),
            "final_offers": sum(
                int(group.get("offer_count") or 1)
