...
def product_availability(product: Dict[str, Any]) -> str:
    ...
    if isinstance(offer, dict) and offer.get("availability"):
        return norm(offer["availability"])
    if product.get("availability"):
        return norm(product["availability"])
...
def product_search_text(product: Dict[str, Any]) -> str:
    values = [
        ...
        product.get("brand"), product.get("source_brand"), product.get("url"),
    ...
    attributes = product.get("attributes")
    if isinstance(attributes, dict):
        values.extend(nested_value(value) for value in attributes.values())
    source = product.get("source")
    if isinstance(source, dict):
        values.extend([
            source.get("source_name"), source.get("source_brand"),
            source.get("name"), source.get("brand"),
        ])
...
def matches(product: Dict[str, Any], query: str) -> bool:
    ...
    query_has_size = bool(re.search(r"\b\d+(?:[.,]\d+)?\s*(?:ml|cl)\b", query_normalized))
    if query_has_size:
        return query_tokens.issubset(text_tokens)

    ignored = {"eau", "de", "parfum", "perfume", "edp", "edt", "extrait", "spray", "for", "by"}
    meaningful = {token for token in query_tokens if token not in ignored}
    return bool(meaningful) and meaningful.issubset(text_tokens)
