...
def product_identity_key(product: Dict[str, Any]) -> tuple:
    ...
    # Once the central matcher has resolved a catalog identity, the same
    # perfume sold by different stores is still one logical result. The
    # store belongs to the offer/price, not to the product identity.
    if variant_id:
        return ("variant", norm(variant_id))

    if product_id:
        return ("product", norm(product_id), norm(product.get("name", "")))

    if gtin:
        return ("gtin", norm(gtin))
...
def _validate_candidate(
    ...
    # If the central matcher cannot resolve the candidate to an
    # authoritative catalog identity, do not fall back to the raw retailer
    # title. That fallback is what lets inconsistent retailer names bypass
    # the canonical title/identity pipeline.
    if matched_product is None:
        return None

    if not _catalog_identity_matches_query(
        matched_product,
        query,
    ):
        return None
