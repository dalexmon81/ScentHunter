...
def matches(
    ...
    """
    Valida il risultato rispettando la famiglia individuata dal catalogo.

    Quando la query identifica una famiglia/variante, il catalogo decide
    l'appartenenza alla famiglia: questo evita di accettare prodotti che
    condividono soltanto una parte del nome. La variante digitata dall'utente
    non restringe la famiglia alle sole parole della query.
    """
    item = normalize_product(product, query)
    name = norm(item.get("name", ""))
    if not name or is_non_perfume(item) or _is_set_product(item):
        return False
    ...
    # Se il catalogo ha individuato una famiglia, questa è la validazione
    # autorevole. In questo modo una query come "Le Beau Le Parfum" mantiene
    # tutte le varianti della famiglia Le Beau, ma esclude "Beau ..." di
    # altre famiglie e prodotti che condividono soltanto una parte del nome.
    if family_candidates:
        return _catalog_family_match(name, family_candidates)

    # Fallback invariato quando il catalogo non produce candidati.
