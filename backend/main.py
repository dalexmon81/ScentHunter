...
def diagnose_notino_search(q: str):
    ...
    try:
        module = importlib.import_module("scrapers.notino.scraper")
        internal_fn = getattr(module, "_search_internal", None)

        if not callable(internal_fn):
            return {
                "ok": False,
                "store": "notino",
                "query": query,
                "error": "Notino scraper has no _search_internal() function",
            }

        results, report = internal_fn(
            query,
            diagnostic=True,
        )
