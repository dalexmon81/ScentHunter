...
def norm(value: Any) -> str:
    ...
    # Normalizzazione Unicode globale: gli accenti non devono cambiare
    # l'identità della query o del prodotto.
...
def _catalog_brand_candidates(query: str) -> List[str]:
    ...
    # La query completa resta l'identità primaria.
    # Non esiste un elenco di parole considerate "varianti": una parola
    # aggiuntiva può essere parte dell'identità di qualsiasi prodotto.
...
def _catalog_identity_match(
    ...
    """
    Verifica in modo generale la compatibilità di un'identità scoperta con
    le denominazioni conosciute dal catalogo.

    Il catalogo viene usato come evidenza positiva, mai come una whitelist
    che possa far sparire un'offerta già scoperta dallo scraper.
    """
    if not family_candidates:
        return True

    identity_tokens = set(norm(identity_text).split())
    ...
    for candidate in family_candidates:
        ...
        if candidate_tokens and candidate_tokens.issubset(identity_tokens):
            return True

    # L'assenza di una forma nel catalogo non deve trasformarsi in uno
    # scarto: il catalogo può essere incompleto rispetto ai negozi reali.
    return True


def _product_identity_text(product: Dict[str, Any]) -> str:
    """
    Costruisce l'identità del prodotto usando tutti i campi strutturali
    disponibili nello scraper, senza dipendere da nomi di profumi specifici.
    """
    values: List[str] = []

    fields = (
        "brand", "name", "title", "product_name", "product_line",
        "variant", "gender", "audience", "sex", "target_gender",
    )
    for field in fields:
    ...
    if isinstance(source, dict):
        for key in (
            "brand", "source_brand", "source_name", "name", "title",
            "product_line", "variant", "gender", "audience", "sex",
            "target_gender",
        ):
...
def matches(
    ...
    """
    Match centrale basato sull'identità, non su una lista di varianti.

    Regola: tutti i termini significativi della query devono essere presenti
    nell'identità strutturale del prodotto. I termini aggiuntivi non vengono
    automaticamente considerati errori: possono rappresentare una variante
    reale e devono restare disponibili per il raggruppamento finale.
    """
    item = normalize_product(product, query)
    if not item or is_non_perfume(item):
        return False
    ...
    raw_identity = _product_identity_text(product)
    normalized_identity = _product_identity_text(item)
    identity_text = " ".join(
        value for value in (raw_identity, normalized_identity) if value
    )
    identity_tokens = set(norm(identity_text).split())
    ...
    # Il catalogo fornisce solo una conferma positiva quando disponibile.
    # Non viene mai usato come filtro rigido, per evitare la perdita di offerte
    # valide quando un catalogo non contiene ancora una variante.
    return _catalog_identity_match(identity_text, family_candidates)
