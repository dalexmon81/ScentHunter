...
logger = logging.getLogger("scent_hunter")
...
def search_perfume(q: str):
    ...
    # 4 worker: tutti gli store vengono avviati rapidamente senza
    # tornare al carico RAM degli 8 worker contemporanei.
    # Il timeout globale evita che uno scraper bloccato tenga aperta
    # la ricerca indefinitamente.
    ...
    try:
        ...
        logger.warning(
            "SEARCH global timeout reached after %ss",
            search_timeout,
        )
    finally:
        ...
        # Non aspettare gli scraper rimasti bloccati: il client deve
        # ricevere comunque i risultati già raccolti.
...
def search_perfume(query: str) -> Dict[str, Any]:
    ...
    # Tutti gli scraper partono in parallelo, ma lasciamo alla ricerca
    # globale abbastanza tempo per ricevere anche gli store più lenti.
    # Il timeout è globale: gli scraper che hanno già risposto conservano
    # i loro risultati, mentre quelli ancora bloccati vengono marcati timeout.
    ...
    try:
        for future in as_completed(future_to_store, timeout=search_timeout):
