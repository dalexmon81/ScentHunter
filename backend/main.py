...
def _run_store_with_diagnostics(
    store: str,
    query: str,
) -> tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """Discovery generica + diagnostica non distruttiva per singolo store."""
    ...
    for attempt in attempts:
        ...
        try:
            ...
            diagnostic["discovery_errors"].append({
                "attempt": attempt,
                "error": error_text,
            })
            print(
                f"STORE_DISCOVERY_ERROR: store={store} "
                f"attempt={attempt!r} error={error_text}",
                flush=True,
            )
            diagnostic["attempts"].append(attempt_info)
            continue
        ...
        attempt_info["raw_items"] = len(results)
        diagnostic["raw_items"] += len(results)

        for item in results:
            if not isinstance(item, dict):
                attempt_info["invalid_items"] += 1
                continue
            ...
            key = product_identity_key(product)
            if key in seen:
                attempt_info["duplicates"] += 1
                continue

            seen.add(key)
            output.append(product)
            attempt_info["new_candidates"] += 1

        diagnostic["attempts"].append(attempt_info)

    diagnostic["unique_candidates"] = len(output)
...
def run_store(
    ...
    """Compatibilità: restituisce solo il candidate pool dello store."""
    results, _diagnostic = _run_store_with_diagnostics(store, query)
    return results
...
def _orchestrate_results_with_diagnostics(
    candidates: List[Dict[str, Any]],
    query: str,
    discovery_diagnostics: Optional[List[Dict[str, Any]]] = None,
) -> tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """Pipeline centrale + fotografia dei punti in cui i candidati spariscono."""
    candidate_pool = unique_results(candidates)

    diagnostic: Dict[str, Any] = {
        "query": query,
        "candidate_pool_before_dedup": len(candidates),
        "candidate_pool_after_dedup": len(candidate_pool),
        "deduplicated_candidates": max(0, len(candidates) - len(candidate_pool)),
        "candidates": [],
        "validation": {
            "accepted": 0,
            "rejected": 0,
        ...
        "final_results": 0,
        "stores": discovery_diagnostics or [],
    }

    ranked_candidates = _pre_rank_candidates(candidate_pool, query)

    for candidate in ranked_candidates:
        source_name = product_field(
            candidate,
            "name",
            "title",
            "product_name",
        ...
        item_diag: Dict[str, Any] = {
            "store": str(candidate.get("store") or "").strip(),
            "name": source_name,
        ...
        try:
            matched = matches(candidate, query)
        except Exception as exc:
            item_diag["stage"] = "validation_exception"
            item_diag["reason"] = f"{type(exc).__name__}: {exc}"
            diagnostic["validation"]["exceptions"] += 1
            diagnostic["candidates"].append(item_diag)
            continue

        if not matched:
            item_diag["stage"] = "rejected_by_matches"
            item_diag["reason"] = "matches() returned False"
            diagnostic["validation"]["rejected"] += 1
            diagnostic["candidates"].append(item_diag)
            continue

        try:
            identity = _family_registry_identity(candidate, query)
        except Exception as exc:
            item_diag["stage"] = "identity_exception"
            item_diag["reason"] = f"{type(exc).__name__}: {exc}"
            diagnostic["validation"]["exceptions"] += 1
            diagnostic["candidates"].append(item_diag)
            continue
        ...
        item_diag["stage"] = (
            "accepted_with_catalog_identity"
            if identity is not None
            else "accepted_generic"
        )
        if identity is not None:
            item_diag["family_id"] = identity.get("family_id", "")
            item_diag["canonical_name"] = identity.get("canonical_name", "")

        diagnostic["validation"]["accepted"] += 1
        diagnostic["candidates"].append(item_diag)

    validated = _validate_candidates_parallel(ranked_candidates, query)
    final_results = sort_by_price(unique_results(validated))
    diagnostic["final_results"] = len(final_results)

    return final_results, diagnostic
...
def _orchestrate_results(
    ...
    results, _diagnostic = _orchestrate_results_with_diagnostics(
        candidates,
        query,
    )
    return results
...
def _search_job_snapshot(job_id: str) -> Dict[str, Any]:
    with SEARCH_JOBS_LOCK:
        ...
        return {
            ...
            "diagnostics": dict(job.get("diagnostics", {})),
...
def diagnose_search(q: str):
    """Ricerca completa con diagnostica discovery -> validation -> finale."""
    query = str(q or "").strip()
    ...
    try:
        try:
            ...
            for future in completed:
                store = futures[future]
                try:
                    store_results, store_diagnostic = future.result()
                    if isinstance(store_results, list):
                        all_results.extend(store_results)
                    if isinstance(store_diagnostic, dict):
                        discovery_diagnostics.append(store_diagnostic)
                except Exception as exc:
            ...
            for future, store in futures.items():
                if future.done():
                    ...
                    message = "Timeout: ricerca del negozio oltre il limite globale"
    ...
    results, diagnostics = _orchestrate_results_with_diagnostics(
        all_results,
        query,
        discovery_diagnostics,
    )
