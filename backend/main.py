...
def matches(product: Dict[str, Any], query: str) -> bool:
    """
    Filtro prudente:
    - mantiene la ricerca per nome;
    - NON elimina automaticamente le varianti della stessa linea;
    - continua a bloccare kit/set/deodoranti e prodotti non-profumo.
    """
...
def build_search_attempts(store: str, query: str) -> List[str]:
    ...
    if re.search(r"\ble\s+beau\b", normalized):
        add("Le Beau")
        add("Le Beau Le Parfum")
        add("Le Beau Paradise Garden")
        return attempts[:3]
