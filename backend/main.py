...
def run_store(
    ...
    """Esegue una sola ricerca per negozio.

    La ricerca di base deve essere rapida: l'espansione generica precedente
    eseguiva fino a 12 query consecutive sullo stesso negozio e poteva
    bloccare l'intera ricerca. Le famiglie/varianti verranno gestite in un
    passaggio dedicato, senza rallentare la ricerca iniziale.
    """
    del catalog_hints  # mantenuto nella firma per compatibilita con /search
    ...
    try:
        source_results = module.search(raw_query) or []
    except Exception:
        return []
...
def search_perfume(q: str):
    ...
    # La ricerca iniziale usa una sola query per negozio.
    # Le varianti di famiglia saranno aggiunte separatamente, senza rallentare
