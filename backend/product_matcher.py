...
class ProductMatcher:
    ...
    def match(
        ...
        if family is not None:
            ...
            # A family in the registry is an identity aid, not a gate that
            # can make a real catalog product disappear.  If the family
            # resolver cannot use its aliases/brand metadata for this offer,
            # fall through to the same generic catalog matcher used for every
            # other perfume.  The final query check below prevents an
            # unrelated catalog product from being accepted.
            generic = self._match_generic(offer, query, started)
            if generic is not None and self._query_matches_catalog_result(generic, query):
                return generic

            print(
                "SCENTHUNTER: MATCHER_UNRESOLVED "
                f"store={store!r} name={raw_name!r} "
                "method=family_then_generic_no_catalog_match",
                flush=True,
            )
            return None
    ...
    @staticmethod
    def _query_matches_catalog_result(
        result: Dict[str, Any],
        query: str,
    ) -> bool:
        q = catalog_variant_key(query or "")
        if not q:
            return True
        ...
        # The canonical catalog name/aliases must contain the requested
        # identity.  This keeps generic matching from turning a store search
        # for one perfume into a different perfume merely because it scores
        # highly against the raw offer name.
        for value in candidates:
            key = catalog_variant_key(value or "")
            if not key:
                continue
