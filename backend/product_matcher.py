...
Supports the current catalog v2 schema and the older flat schema.
No store-specific or product-specific exceptions belong here.
"""
...
class CatalogProduct:
    ...
    def from_dict(cls, data: Dict[str, Any]):
        ...
        raw_formats = data.get("formats_ml") or []
        if not raw_formats and data.get("size_ml") not in (None, ""):
            raw_formats = [data.get("size_ml")]

        formats = []
        for value in raw_formats:
            try:
                formats.append(float(str(value).replace(",", ".")))
            except (TypeError, ValueError):
                continue

        raw_gtins = data.get("gtins") or data.get("ean") or []
        if isinstance(raw_gtins, (str, int, float)):
            raw_gtins = [raw_gtins]

        raw_mpns = data.get("mpns") or data.get("mpn") or []
        if isinstance(raw_mpns, (str, int, float)):
            raw_mpns = [raw_mpns]
    ...
    def normalized_name(self):
        return _name_for_matching(self.name)

    @property
    def normalized_aliases(self):
        return tuple(_name_for_matching(x) for x in self.aliases)
...
class ProductMatcher:
    ...
    def __init__(self, catalog: Iterable[Dict[str, Any] | CatalogProduct]):
        ...
        self.catalog = [x for x in self.catalog if x.catalog_id and x.name]
    ...
    def _best_match(self, offer):
        ...
        if gtin and gtin in self._by_gtin and len(self._by_gtin[gtin]) == 1:
            return self._by_gtin[gtin][0], "gtin", 1.0

        mpn = identifier(offer, self.MPN_KEYS)
        if mpn and mpn in self._by_mpn and len(self._by_mpn[mpn]) == 1:
        ...
        for candidate in (product.normalized_name, *product.normalized_aliases):
            ...
            if candidate in name:
                ...
                generic_extras = {
                    ...
                    "spray", "men", "man", "male", "women", "woman",
                    "femme", "homme", "for", "by", "vaporisateur", "natural",
                }
                if extra_tokens.issubset(generic_extras):
                    # Prefer the most specific catalog alias when two families
                    # share the same base name.
                    specificity_bonus = min(len(candidate_tokens), 12) * 0.001
                    f_score = max(f_score, 0.92 + specificity_bonus)
