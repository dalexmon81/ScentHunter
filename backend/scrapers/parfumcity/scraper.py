# backend/scrapers/parfumcity/scraper.py
"""
ParfumCity scraper - versione corretta con primary + fallback deterministico.
"""
import requests
...
BASE_URL = "https://www.parfumcity.de"
SEARCH_URL = "https://www.parfumcity.de/index.php" 
...
def extract_candidate_urls_from_search(html: str, query: str) -> List[str]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "parfumcity.de" in href and query.lower() in text:
...
def parse_product_page(html: str, product_url: str) -> Optional[Dict]:
    ...
    price_el = soup.find("span", class_=re.compile(r"price|preis", re.I))
    if price_el:
    ...
    if not name:
        return None
    return {"store": "parfumcity", "name": name, "price": price, "currency": "EUR", "in_stock": True, "url": product_url}
...
def fallback_search(query: str) -> List[Dict]:
    fallback_url = "https://www.parfumcity.de/index.php"
    params = {"search": query, "s": query}
    try:
        resp = requests.get(fallback_url, params=params, headers=HEADERS, timeout=(CONNECT_TIMEOUT, READ_TIMEOUT))
    except Exception:
        return []
    if is_blocked_or_invalid_response(resp):
        return []
