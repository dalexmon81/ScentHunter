...
def price(v):
    ...
    raw=m.group(0)
    try:
        n=float(raw.replace(",",".")); n/=100 if re.fullmatch(r"\d+",raw) and n>=100 else 1
        return round(n,2)
    except ValueError:
        return None
...
def _urls_from_sitemap(session,q,limit=80):
    ...
    try:
        ...
        while queue and len(urls)<limit:
            sm=queue.pop(0); x=_get(session,sm)
            if not x: continue
...
def _urls_from_shopify_catalog(session,q,limit=120):
    ...
    def add_product(p):
        ...
        if not matches(f"{title} {vendor} {handle}",q) or not handle: return
        u=BASE_URL+"/products/"+handle
        if u not in seen: seen.add(u); urls.append(u)
    endpoints=[BASE_URL+"/products.json",BASE_URL+"/collections/all/products.json"]
...
def _discover(session,q):
    ...
    r=_get(session,BASE_URL+"/search/suggest.json",{
        "q":q,"resources[type]":"product","resources[limit]":20,
    ...
    if r:
        try:
            ...
            for product in products:
                ...
                if matches(f"{product.get('title','')} {product.get('vendor','')} {u or ''}",q): add(u)
        except (ValueError,TypeError): pass
    if urls: return urls[:8]
    r=_get(session,BASE_URL+"/search",{"q":q,"type":"product"})
    if r:
        soup=BeautifulSoup(r.text,"html.parser")
        for a in soup.select('a[href*="/products/"]'):
            u=a.get("href"); text=f"{a.get('title','')} {a.get_text(' ',strip=True)} {u or ''}"
            if matches(text,q):
                add(u)
                if len(urls)>=8: break
    return urls[:8]
...
# FIX: Orioudh titles contain commercial suffixes such as
# "Hawas for him by Rasasi - Men Perfume - EDP - 100 ml".
# ProductMatcher needs the perfume identity, while the original retailer
# title must remain available in source.source_name.
def _identity_name(product,variant):
    title=clean(product.get("title"))
    vname=clean(variant.get("title"))
    s=title if not vname or vname=="Default Title" else f"{title} {vname}"
    s=re.sub(r"\s+by\s+[A-Za-z0-9&.'’ -]+(?=\s+-|$)","",s,flags=re.I)
    s=re.sub(r"\s*-\s*(?:men|women|man|woman|male|female|homme|femme|herren|damen)\s+perfume\b","",s,flags=re.I)
    s=re.sub(r"\s*-\s*(?:edp|edt|extrait(?:\s+de\s+parfum)?|eau\s+de\s+parfum|eau\s+de\s+toilette|parfum)\b","",s,flags=re.I)
    s=re.sub(r"\s*-\s*\d+(?:[.,]\d+)?\s*(?:ml|cl)\b","",s,flags=re.I)
    return clean(s).strip(" -")

def _item(product,variant,url):
    name=clean(product.get("title")); vname=clean(variant.get("title"))
    product_type=clean(product.get("product_type"))
    haystack=norm(f"{name} {vname} {product_type}")
    if re.search(r"\bmystery\s+box\b|\bgift\s+set\b",haystack): return None
    source_name=name if not vname or vname=="Default Title" else f"{name} {vname}"
    if not matches(f"{name} {product.get('vendor','')} {url}",CURRENT_QUERY): return None
    p=price(variant.get("price"))
    if p is None: return None
    size=size_ml(vname,name); conc=concentration(vname,name)
    ...
    if not image:
        imgs=product.get("images") or []; image=imgs[0] if imgs else None
    return {
        "store":STORE,
        ...
        # IMPORTANT: only this field is normalized for ProductMatcher.
        "name":_identity_name(product,variant),
...
def search(query):
    ...
    try: discovered=_discover(session,CURRENT_QUERY)[:8]
    finally: session.close()
    if not discovered: return []
    from concurrent.futures import ThreadPoolExecutor,as_completed
    def fetch_one(url):
        try: r=requests.get(url.rstrip("/")+".js",headers=HEADERS,timeout=TIMEOUT)
    ...
    with ThreadPoolExecutor(max_workers=min(4,len(discovered))) as pool:
        futures={pool.submit(fetch_one,url):url for url in discovered}
        for future in as_completed(futures):
            url=futures[future]; data=future.result()
            ...
            for variant in data.get("variants") or []:
                ...
                canonical_url=re.sub(r"^https?://www\.orioudh\.com",BASE_URL,item["url"].rstrip("/"),flags=re.I)
                key=(canonical_url,variant_id)
                if key in seen: continue
                seen.add(key); item["url"]=canonical_url; item["source"]["url"]=canonical_url; out.append(item)
    return out
...
def diagnose(query):
    global CURRENT_QUERY
    CURRENT_QUERY=clean(query); session=requests.Session()
    try:
...
if __name__=="__main__":
    import argparse
    parser=argparse.ArgumentParser(); parser.add_argument("query"); parser.add_argument("--diagnose",action="store_true")
