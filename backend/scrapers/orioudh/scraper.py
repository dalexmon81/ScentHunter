...
def _extract_image_url(value):
    """Extract a usable image URL from Shopify image values."""
    if not value:
        return None
    ...
    if isinstance(value, dict):
        for key in ("src", "url", "original_src", "contentUrl", "image", "featured_image"):
            candidate = _extract_image_url(value.get(key))
            if candidate:
                return candidate
        srcset = value.get("srcset")
        if isinstance(srcset, str) and srcset.strip():
            return clean(srcset.split(",")[0].strip().split(" ")[0])
        return None
...
def _page_image(session, url, product_name):
    """Fallback to the real product page when Shopify JSON has no usable image."""
    r = _get(session, url)
    if not r:
        return None
    try:
    ...
    # Shopify product pages normally expose the main product image here.
    for attrs in (
        {"property": "og:image"},
        ...
        candidate = _extract_image_url(tag.get("content") if tag else None)
        if candidate:
            return urljoin(BASE_URL, candidate)

    # Last fallback: choose an image whose alt/title is actually about the product,
    # avoiding logos and generic theme images.
    product_tokens = [t for t in norm(product_name).split() if len(t) >= 3]
    best = None
    best_score = 0
    for img in soup.find_all("img"):
        candidate = _extract_image_url(img.get("src") or img.get("data-src") or img.get("data-original"))
        if not candidate:
            continue
        context = norm(" ".join([
            clean(img.get("alt")),
        ...
        score = sum(1 for token in product_tokens if token in context)
        if score > best_score:
            best_score = score
...
def _item(product,variant,url,session):
    ...
    # Prefer the variant image, then Shopify product images. Some Shopify
    # payloads expose these as dicts rather than plain strings.
    image=_extract_image_url(variant.get("featured_image"))
    if not image:
