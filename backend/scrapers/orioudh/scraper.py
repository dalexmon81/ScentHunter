# backend/scrapers/orioudh/scraper.py
"""
OrioUdh scraper - versione corretta con primary + fallback deterministico.
"""
import requests
...
BASE_URL = "https://www.orioudh.com"
SEARCH_URL = "https://www.orioudh.com/search"
...
def extract_candidate_urls_from_search(html: str, query: str) -> List[str]:
    soup = BeautifulSoup(html, "html.parser")
    urls = []
    for a in soup.find_all("a", href=True):
        href = a["href"]
        text = a.get_text(strip=True).lower()
        if "orioudh.com" in href or href.startswith("/") and query.lower() in text:
...
def parse_product_page(html: str, product_url: str) -> Optional[Dict]:
    ...
    price_el = soup.find("span", class_=re.compile(r"price|prezzo|eur", re.I))
    if price_el:
    ...
    if not name:
        return None
    return {"store": "orioudh", "name": name, "price": price, "currency": "EUR", "in_stock": True, "url": product_url}
...
def primary_search(query: str) -> List[Dict]:
    ...
    if is_blocked_or_invalid_response(resp):
        return []
    candidate_urls = extract_candidate_urls_from_search(resp.text, query)
...
def fallback_search(query: str) -> List[Dict]:
    # Fallback: pagina collezione o ricerca alternativa
    fallback_url = "https://www.orioudh.com/collections/all"
    try:
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"]
        text = a.get_text(strip=True).lower()
        if ("/products/" in href or "/collection/" in href) and query.lower() in text:
