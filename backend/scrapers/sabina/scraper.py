...
def _parse_datalayer_impressions(text, query):
    """Extract generic product impressions embedded in Sabina dataLayer.

    Sabina's current search page publishes the product cards in JavaScript
    telemetry even when the visible HTML card structure changes. We use only
    runtime product fields (name/id/price/brand/variant) and resolve the
    corresponding product URL from the same response. No product-specific
    seed or rule is used.
    """
    source = str(text or "")
    if not source:
        return []
    ...
    # Product links available in the same HTML, indexed by the numeric product id.
    id_to_url = {}
    for a in soup.find_all("a", href=True):
        ...
        if m:
            id_to_url.setdefault(m.group(1), href)

    # Also accept data-href/product-image URLs used by the current cards.
    for node in soup.find_all(attrs={"data-href": True}):
        href = urljoin(BASE, node.get("data-href") or "").split("#", 1)[0].split("?", 1)[0]
        if not _looks_like_product_url(href):
            continue
        m = re.search(r"/(\d+)-", href)
        if m:
    ...
    # The current dataLayer impression objects are JSON-like and use quoted
    # fields. Extract each impression conservatively from the script text.
    pattern = re.compile(
        r'\{\s*"name"\s*:\s*"(?P<name>(?:\\.|[^"\\])*)"'
        r'.{0,1200}?"id"\s*:\s*"(?:[^"\\]*?)(?P<id>\d+)_[^"\\]*"'
        r'.{0,500}?"price"\s*:\s*"?(?P<price>\d{1,4}(?:[.,]\d{2})?)"?'
        r'(?P<tail>.{0,500}?)\}',
        re.I | re.S,
    )
...
def _parse_html(text, query):
    ...
    # First use Sabina's embedded search impressions. This is the current
    # authoritative search payload when the card markup is not parser-stable.
    rows.extend(_parse_datalayer_impressions(text, query))
