...
def _card_name(card):
    ...
    # Prefer explicit product-name candidates before generic card text.
    cleaned = []
    ...
    # Prefer candidates that look like actual product names. Specific
    # product-name selectors are collected before generic containers/links.
    # Keep the first strong candidate instead of selecting the shortest
    # arbitrary string from the whole card.
...
def _extract_anchor_products(soup, base_url):
    ...
    for link in soup.find_all("a", href=True):
        ...
        # Prefer explicit product-name selectors, then the anchor label.
        name = ""
        for selector in (
            ".product-name",
            ".product-title",
            ...
            if element:
                ...
                if value and not PRICE_RE.fullmatch(value):
                    name = value
                    break
...
def _parse_page(html, base_url, query):
    ...
    for row in merged:
        # Primary matching must use the product identity itself: the
        # extracted name and the product URL slug. Card text is deliberately
        # not used as the primary all-token match because it may contain
        # category text, neighbouring products, badges or duplicated content.
        name_score = _score(query, row.get("name") or "")
        ...
        # Card text remains only a secondary signal. It can strengthen a
        # candidate already supported by its name/slug, but can never create
        # a match by itself.
        text_score = _score(query, row.get("text") or "")
...
def search(query):
    ...
    try:
        ...
        while queue and len(visited) < MAX_PAGES:
            ...
            # Do not stop at the first weak/ambiguous card match. The
            # discovery phase must be pagination-aware and generic. Stop only
            # when at least one candidate is strongly identified by its
            # product name or URL slug; otherwise keep following the site's
            # pagination graph until MAX_PAGES.
            strong_candidates = [
                candidate
                for candidate in candidates
                if (
                    candidate.get("name_score", 0.0) >= 1.0
                    or candidate.get("slug_score", 0.0) >= 1.0
                )
            ]

            if strong_candidates:
                # We have a product-identity match. Verification will inspect
                # the strongest candidates and validate the real product page.
                break
