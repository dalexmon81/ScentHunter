...
if featured_image:
    candidates.append(featured_image)
...
if isinstance(variant, dict):
    ...
    if variant_featured_image:
        candidates.append(variant_featured_image)
    variant_image = variant.get("image")
    if variant_image:
        candidates.append(variant_image)
    variant_image_id = variant.get("featured_image_id")
    if variant_image_id and isinstance(images, list):
        for image_item in images:
            if isinstance(image_item, dict):
                if str(image_item.get("id") or "") == str(variant_image_id):
                    candidates.insert(0, image_item)
