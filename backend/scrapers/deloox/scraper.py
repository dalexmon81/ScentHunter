...
def _url_matches_query(product_url, query):
    """
    L'URL serve solo come filtro di compatibilità, non come prova
    definitiva del nome del prodotto: Deloox può usare slug abbreviati.
    La verifica precisa viene fatta sul testo della card/pagina.
    """
    url_tokens = set(
        _tokens(product_url)
    )
    ...
    # Evita prodotti completamente estranei, ma non richiede
    # che ogni parola della query compaia nello slug dell'URL.
    return overlap >= max(
        1,
