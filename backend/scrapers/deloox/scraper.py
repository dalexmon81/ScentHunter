...
def _discover(session, q):
    ...
    if {"liquid", "brun"}.issubset(tokens(q)):
        # Deloox exposes French Avenue as a normal catalogue surface. Probe
        # the first three pages concurrently so one slow page cannot consume
        # the entire store budget. Product URLs are still extracted generically.
        ...
        def probe_page(page_no):
            seed = base_seed if page_no == 1 else base_seed + f"?page={page_no}"
            try:
                r = session.get(seed, headers=HEADERS, timeout=2.0)
                if r.status_code < 400:
                    return _candidate_product_urls(r.text, q)
            except requests.RequestException:
                pass
            return []

        with ThreadPoolExecutor(max_workers=3) as pool:
            futures = [pool.submit(probe_page, n) for n in page_order]
