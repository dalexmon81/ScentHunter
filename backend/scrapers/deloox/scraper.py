...
def _product(url, html, query):
    ...
    # Deloox variant URLs contain the concrete bottle size even when the
    # page metadata omits it (e.g. ...-50-ml.html / ...-100-ml.html).
    if selected_size is None:
        url_size = re.search(r"(?:-|/)(\d{1,4})-ml(?:\.html)?(?:$|[?#])", url, re.I)
        if url_size:
            selected_size = int(url_size.group(1))
