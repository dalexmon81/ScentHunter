...
def _extract_product_urls(html: str, query: str, allow_opaque=False):
    ...
    def add(raw_url, context=""):
        ...
        if score == 0 and not allow_opaque:
            # keep a small number of opaque candidates only when allowed
            return
        seen.add(url)
        candidates.append((score, url))

    # try anchors first (most common)
    for a in soup.find_all("a", href=True):
        add(a.get("href"), a.get_text(" ", strip=True))

    # fallback: regex scan for common product:url occurrences in inline JS or HTML
    raw = (html or "").replace("\\/", "/")
    patterns = [
    ...
    # final sort: prefer higher score and shorter url
