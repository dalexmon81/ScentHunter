...
def _looks_like_product_url(url):
    ...
    # Notino product URLs are normally multi-segment paths.
...
def _product(url, html, query):
    ...
    if not name or not matches(name, query):
        # Some search cards expose a more complete title only in nearby
        # structured data. Try all Product JSON-LD objects before rejecting.
...
def _candidate_product_urls(html, query):
    """Extract generic Notino product URLs from HTML/JSON/scripts."""
    ...
    def add(raw_url, context=""):
        ...
        # Discovery must be generic: candidate must be connected to
        # the requested query by visible context or URL.
