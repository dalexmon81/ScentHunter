...
TIMEOUT = int(os.getenv("NOTINO_TIMEOUT_S", "20"))
...
def _candidate_product_urls(html, query):
    ...
    # Search result cards / normal anchors.
    ...
    # Last generic pass for embedded /p-<id>/ URLs.
...
def _search_page_with_playwright(query):
    if sync_playwright is None or not BROWSER_ENABLED:
        return None
    ...
    try:
        ...
        LOGGER.warning("Notino search browser error: %s", exc)
        return None


def _discover(session, query):
    # Deloox principle: the search page is discovery.
    # Do not validate product names until the product page is fetched.
    response, html = _search_page(session, query)
    if html:
        urls = _candidate_product_urls(html, query)
        if urls:
            return urls

    # Only if direct HTTP yielded no candidates, use the real search page
    # through a browser. Still the same discovery source.
    html = _search_page_with_playwright(query)
    if html:
        return _candidate_product_urls(html, query)
    ...
    try:
        for url in _discover(session, query):
            ...
            try:
                ...
                if response.status_code < 400 and "notino.fr" in response.url:
                    html = response.text
            except requests.RequestException:
                pass
            ...
            key = (str(item.get("url", "")).lower(), norm(item.get("name", "")))
            if key in seen:
                continue
...
def diagnostic_discovery(query):
    ...
    try:
        response, html = _search_page(session, query)
        candidates = _candidate_product_urls(html, query) if html else []
        return {
            "store": STORE.lower(),
            "query": query,
...
if __name__ == "__main__":
    ...
    payload = diagnostic_discovery(args.query) if args.diagnostic else search(args.query)
