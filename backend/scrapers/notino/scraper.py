...
def extract_variant_offers(soup):
    """Extract explicit size/price pairs exposed by a Notino product page.

    This is deliberately conservative: a size is accepted only when a price
    is present in the same small page element or structured object.  We never
    infer a size from the position of a price or from the requested query.
    """
    variants = []
    seen = set()
    ...
    def add(size, price):
        try:
            size = int(float(str(size).replace(",", ".")))
        except Exception:
            return
    ...
    def inspect_text(text):
        ...
        if not text or len(text) > 700:
            return
        sizes = list(SIZE_RE.finditer(text))
        if not sizes:
            return
        prices = list(re.finditer(r"(?<!\d)(\d{1,4}(?:[.,]\d{2}))\s*€?", text))
        if len(sizes) == 1 and prices:
            # Prefer the closest price to the explicit size.
            sm = sizes[0]
            price = min(prices, key=lambda m: abs(m.start() - sm.end()))
            add(sm.group(1), price.group(1))
        elif len(sizes) == len(prices):
            for sm, pm in zip(sizes, prices):
                add(sm.group(1), pm.group(1))

    # Variant controls are usually the cleanest representation.
    for el in soup.find_all(["option", "label", "button"]):
        inspect_text(el.get_text(" ", strip=True))
        for attr in ("aria-label", "data-label", "data-value", "value"):
            inspect_text(el.get(attr, ""))

    # Some Notino builds expose variant data in compact JSON/script blobs.
    for script in soup.find_all("script"):
        raw = script.string or script.get_text()
        if not raw or len(raw) > 250000:
            continue
        # Local windows avoid pairing unrelated prices/sizes from the whole
        # application state.
        for m in re.finditer(r"(?<!\d)(\d{2,4})\s*ml\b", raw, re.I):
            window = raw[m.end():m.end()+220]
            pm = re.search(r"(?<!\d)(\d{1,4}(?:[.,]\d{2}))\s*€?", window)
            if pm:
                add(m.group(1), pm.group(1))

    # Visible page text fallback, still requiring a nearby price.
    visible = soup.get_text(" ", strip=True)
    for m in re.finditer(r"(?<!\d)(\d{2,4})\s*ml\b", visible, re.I):
        window = visible[m.end():m.end()+180]
        pm = re.search(r"(?<!\d)(\d{1,4}(?:[.,]\d{2}))\s*€", window)
        if pm:
            add(m.group(1), pm.group(1))
...
def search(query):
    ...
    try:
        ...
        for url in candidates:
            ...
            variants = extract_variant_offers(BeautifulSoup(product_response.text, "html.parser"))
            if variants:
                for variant_size, variant_price in variants:
                    item = dict(product)
                    item["size_ml"] = variant_size
                    item["sizeml"] = variant_size
                    item["size"] = variant_size
                    item["price"] = f"{variant_price:.2f}".replace(".", ",") + " €"
                    item["available"] = product.get("available")
                    item["offer"] = dict(item.get("offer") or {})
                    item["offer"]["price"] = variant_price
                    item["attributes"] = dict(item.get("attributes") or {})
                    item["attributes"]["size_ml"] = {"value": variant_size, "source": "variant_page"}
                    key = (item["url"].lower(), norm(item["name"]), variant_size, variant_price)
                    if key not in seen:
                        seen.add(key)
                        results.append(item)
            else:
                key = (product["url"].lower(), norm(product["name"]), product.get("size_ml"))
                if key not in seen:
                    seen.add(key)
