...
URL_IDENTITY_IGNORED = {
    # Generic format/concentration words are often absent from Notino URL
    # slugs and therefore cannot be required for URL identity validation.
    "eau", "de", "parfum", "perfume", "edp", "edt", "edc",
    "extrait", "spray", "vaporisateur", "ml", "cl", "dl", "l",
    "oz", "fl", "for", "by",
}

URL_IDENTITY_EQUIVALENTS = (
    frozenset({"him", "man", "men", "homme", "heren", "mannen", "male"}),
    frozenset({"her", "woman", "women", "femme", "dames", "vrouwen", "female"}),
)


def _url_identity_tokens(value: Any) -> List[str]:
    return [
        token
        for token in _query_tokens(value)
        if token not in URL_IDENTITY_IGNORED
    ]


def _url_matches_query_identity(url: str, query: str) -> bool:
    """Require every identity-bearing query token to exist in the URL slug.

    Notino search pages can expose shared card text next to unrelated product
    URLs. The URL itself must therefore confirm the requested product identity
    before that URL is allowed into the candidate pool. Generic format words
    are ignored because they are commonly omitted from SEO slugs.
    """
    query_tokens = _url_identity_tokens(query)
    if not query_tokens:
        return False

    brand = _brand_from_product_url(url)
    slug_name = _name_from_product_url(url)
    url_tokens = set(_url_identity_tokens(f"{brand} {slug_name}"))
    for token in query_tokens:
        if token in url_tokens:
            continue
        equivalent = next(
            (group for group in URL_IDENTITY_EQUIVALENTS if token in group),
            None,
        )
        if equivalent is None or not (equivalent & url_tokens):
            return False
    return True
...
def _make_candidate(url: str, anchor: str, card: str, query: str, source: str) -> Optional[Dict[str, Any]]:
    ...
    # The reader can expose a search-page heading as the anchor text for
    # several different product URLs. Never let that shared search context
    # manufacture an identity for an unrelated URL. The URL slug must contain
    # every identity-bearing query token before the candidate is accepted.
...
def _display_product_name(name: Any, url: str = "", brand_hint: str = "") -> str:
    """Return one consistent catalog display name: ``Brand - Variant``.

    This is presentation-only. Discovery/matching continues to use the raw
    product name, so adding the brand separator cannot change which products
    are found. The brand is taken from structured product data when available,
    otherwise from the first Notino product-URL path segment. If the raw name
    already starts with the brand, that prefix is removed before formatting.
    The rule is intentionally generic and applies to every brand/product.
    """
    raw_name = _clean_name(str(name or ""))
    if not raw_name:
        return ""
    ...
    # Normalize an already formatted name first.
    raw_name = re.sub(r"^\s*([^:]+?)\s*[-–—:]\s*", r"\1 - ", raw_name)

    # Remove a leading brand from the product name, case-insensitively.
    prefix = re.compile(r"^" + re.escape(brand) + r"(?:\s*[-–—:]\s*|\s+)", re.I)
    variant = prefix.sub("", raw_name, count=1).strip()

    # If the name is only the brand, do not manufacture an empty variant.
    if not variant or variant.casefold() == brand.casefold():
        return brand
...
def _image_url(value: Any, base_url: str = BASE_URL) -> str:
...
def _extract_page_image(soup: BeautifulSoup, product_name: str, product_url: str) -> str:
    """Select the image belonging to the current Notino product page."""
    target = _product_norm(product_name)
    target_tokens = set(_query_tokens(product_name))
    candidates: List[Tuple[int, int, str]] = []
    seen = set()

    def add(value: Any, context: Any = "", base_score: int = 0) -> None:
        url = _image_url(value)
        if not url or url in seen:
            return
        seen.add(url)
        ctx = _product_norm(context)
        ctx_tokens = set(_query_tokens(ctx))
        score = base_score + 2 * len(target_tokens & ctx_tokens)
        if target and target in ctx:
            score += 25
        # Prefer URLs that visibly carry the product identity.
        url_text = _product_norm(url)
        if target and target in url_text:
            score += 20
        if any(marker in url_text for marker in ("logo", "icon", "sprite", "banner")):
            score -= 30
        candidates.append((score, -len(candidates), url))

    for product in _json_ld_products(soup):
        product_name_json = _clean(product.get("name"))
        product_url_json = str(product.get("url") or product.get("@id") or "")
        same = False
        if product_url_json and product_url_json.rstrip("/") == product_url.rstrip("/"):
            same = True
        elif product_name_json and target:
            same = _product_norm(product_name_json) == target
        if not same:
            continue
...
def _extract_reader_image(text: str, product_name: str) -> str:
    raw = html_lib.unescape(str(text or "")).replace("\\/", "/")
    target = _product_norm(product_name)
    ...
    for match in markdown.finditer(raw):
        alt = _product_norm(match.group(1))
        url = _image_url(match.group(2))
        if not url:
            continue
        score = 10 + (25 if target and target in alt else 0)
        if score > best[0]:
            best = (score, url)

    for line in raw.splitlines():
        match = re.match(r"\s*(?:image|image url|og:image)\s*:\s*(https?://\S+)", line, re.I)
        if not match:
            continue
        url = _image_url(match.group(1))
        if url:
            score = 20
            if score > best[0]:
                best = (score, url)
...
def _reader_product(
    ...
    return {
        ...
        "price": price,
        "url": candidate_url,
        "image": _extract_reader_image(raw, name),
    }
...
def _product_details(
    ...
    return {
        "store": STORE,
        "name": _display_product_name(name, final_url, brand),
        "price": price,
        "url": final_url,
        "image": _extract_page_image(soup, name, final_url),
    } if price else None
