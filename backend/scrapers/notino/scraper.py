...
TIMEOUT = 20
READER_TIMEOUT = 12
SCRAPER_VERSION = "notino-FR-generic-discovery-2026-08-21-v13-contamination-filter"
...
def _make_candidate(
    ...
    """Build a candidate while preventing Notino page-text contamination.

    Notino/Jina can occasionally return an <a> whose visible text contains
    several neighbouring products. In that case the URL slug is safer than
    the giant anchor. A candidate is rejected when its structured URL name
    clearly belongs to another product.
    """
    ...
    # A search-card anchor should be a product title, not an entire page.
    # Keep normal long perfume names, but treat giant SEO/reader blobs as
    # contaminated text.
    slug_name = _name_from_product_url(url)
    url_brand = _brand_from_product_url(url)
    branded_slug = _clean_name(f"{url_brand} {slug_name}") if url_brand and slug_name else slug_name

    anchor_name = _clean_name(anchor)
    name = anchor_name

    # If the anchor is clearly polluted, prefer the structured URL slug.
    polluted_anchor = (
        len(anchor_name) > 220
    ...
    # If the URL slug is available and it clearly belongs to another
    # product, reject the candidate even when the anchor happened to contain
    # the requested words because of neighbouring-page text.
    if slug_name:
        slug_matches = _fuzzy_query_match(
            f"{url_brand} {slug_name}" if url_brand else slug_name,
            query,
        )[0]
        if not slug_matches:
            return None

        # For clean product URLs, prefer the URL-derived product name.
        if polluted_anchor or not _fuzzy_query_match(name, query)[0]:
            name = branded_slug or slug_name
...
def _card_result(candidate: Dict[str, Any], query: str) -> Optional[Dict[str, Any]]:
    ...
    # URL is authoritative when it exposes an SEO product name.
    if slug_name:
        url_name = _clean_name(f"{brand} {slug_name}" if brand else slug_name)
        if not _fuzzy_query_match(url_name, query)[0]:
            return None
        name = url_name
    else:
        name = _clean_name(anchor)

    # Do not let a whole search-page blob become the product title.
    if len(name) > 220 or name.count("##") >= 2:
        if slug_name:
            name = _clean_name(f"{brand} {slug_name}" if brand else slug_name)
        else:
            return None
...
def _product_details(
    ...
    # A redirected/final SEO URL must still describe the requested product.
    final_slug = _name_from_product_url(final_url)
    final_brand = _brand_from_product_url(final_url)
    if final_slug:
        final_name = _clean_name(
            f"{final_brand} {final_slug}" if final_brand else final_slug
        )
        if not _fuzzy_query_match(final_name, query)[0]:
            return None
